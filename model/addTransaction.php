...
$db = connectDB();
$query = $db->prepare("INSERT INTO `transactions` (`date`, `amount`, `details`, `nextMonth`, `effectiveDate`, `idCategory`) VALUES (?,?,?,?,?,?)");
$query->bindParam(1, $date);
$query->bindParam(2, $amount);
$query->bindParam(3, $details);
$query->bindParam(4, $next);
$query->bindParam(5, $effectiveDate);
$query->bindParam(6, $idCategory);
